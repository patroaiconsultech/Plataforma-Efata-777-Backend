...
class Settings(BaseSettings):
    ...
    legacy_claim_enabled: bool = Field(False, alias="PLATFORM_LEGACY_CLAIM_ENABLED")
    legacy_database_url: str = Field("", alias="LEGACY_DATABASE_URL")
    legacy_claim_source_name: str = Field("orkio_legacy", alias="PLATFORM_LEGACY_CLAIM_SOURCE_NAME")
    legacy_claim_tenant_map_json: str = Field("{}", alias="PLATFORM_LEGACY_CLAIM_TENANT_MAP_JSON")
    legacy_claim_require_verified_email: bool = Field(True, alias="PLATFORM_LEGACY_CLAIM_REQUIRE_VERIFIED_EMAIL")
    legacy_claim_max_threads: int = Field(50, alias="PLATFORM_LEGACY_CLAIM_MAX_THREADS")
    legacy_claim_max_messages: int = Field(5000, alias="PLATFORM_LEGACY_CLAIM_MAX_MESSAGES")
    legacy_claim_max_files: int = Field(100, alias="PLATFORM_LEGACY_CLAIM_MAX_FILES")
    legacy_claim_max_bytes: int = Field(50_000_000, alias="PLATFORM_LEGACY_CLAIM_MAX_BYTES")
    ...
    def secure_modes(self):
        if self.legacy_claim_enabled:
            if not self.legacy_database_url.strip():
                raise ValueError("LEGACY_DATABASE_URL_REQUIRED")
            if not self.legacy_claim_source_name.strip():
                raise ValueError("LEGACY_CLAIM_SOURCE_NAME_REQUIRED")
            if min(
                self.legacy_claim_max_threads,
                self.legacy_claim_max_messages,
                self.legacy_claim_max_files,
                self.legacy_claim_max_bytes,
            ) <= 0:
                raise ValueError("LEGACY_CLAIM_LIMIT_INVALID")
            if self.environment in {"staging", "production"}:
                if not self.legacy_database_url.lower().startswith(("postgresql://", "postgres://", "postgresql+psycopg://")):
                    raise ValueError("LEGACY_DATABASE_POSTGRES_REQUIRED")
                if self.legacy_database_url.strip() == self.database_url.strip():
                    raise ValueError("LEGACY_DATABASE_MUST_BE_SEPARATE")
